...
import ParisArrondissement3D from '../components/dashboard/ParisArrondissement3D';
import BuildingsBubbleChart from '../components/dashboard/BuildingsBubbleChart';
...
export default function DashboardPage() {
  ...
  // State for View toggles (Graphique, Bulles, 3D, Table)
  ...
  // Données formatées pour la 3D Three.js et les Bulles 2D
  ...
  useEffect(() => {
    ...
    // --- BÂTIMENTS ---
    if (data.buildings && data.buildings.length > 0 && viewModes.batiment === 'chart') {
    ...
    // --- TYPES ---
    if (data.types && data.types.length > 0 && viewModes.types === 'chart') {
    ...
    // --- DPE ---
    if (data.dpe && data.dpe.length > 0 && viewModes.dpe === 'chart') {
    ...
    <main className="main-content">
      <header className="mb-10 flex items-center justify-between">
        <div className="flex flex-col">
          <span className="text-xs font-bold text-slate-400 mb-1" id="viewSubtitle">Synthèse Interactive Multi-Dimensions</span>
          <h1 className="text-2xl font-bold text-slate-800" id="viewTitle">Tableau de Bord &amp; Visualisation 3D / 2D</h1>
        </div>
      ...
      <div id="dashboardContent" className="space-y-8">
        ...
        <section id="section-batiment" className={`view-section ${route !== 'dashboard' && route !== 'batiment' ? 'hidden' : ''}`}>
          <div className="flex flex-wrap items-center justify-between gap-4 mb-8">
            ...
            {/* Multi-View Mode Switcher (Graphique, Bulles 2D, Carte 3D, Données) */}
            <div className="bg-slate-200 p-1 rounded-xl flex flex-wrap text-xs font-semibold text-slate-600 gap-1">
                <button
                  type="button"
                  className={`px-3 py-1.5 rounded-lg transition-all flex items-center gap-1.5 ${viewModes.batiment === 'chart' ? 'active text-primary bg-white shadow-sm font-bold' : 'hover:text-slate-900'}`}
                  onClick={() => toggleView('batiment', 'chart')}
                >
                ...
                <button
                  type="button"
                  className={`px-3 py-1.5 rounded-lg transition-all flex items-center gap-1.5 ${viewModes.batiment === 'bubbles' ? 'active text-primary bg-white shadow-sm font-bold' : 'hover:text-slate-900'}`}
                  onClick={() => toggleView('batiment', 'bubbles')}
                >
                ...
                <button
                  type="button"
                  className={`px-3 py-1.5 rounded-lg transition-all flex items-center gap-1.5 ${viewModes.batiment === '3d' ? 'active text-cyan-600 bg-white shadow-sm font-bold' : 'hover:text-slate-900'}`}
                  onClick={() => toggleView('batiment', '3d')}
                >
                ...
                <button
                  type="button"
                  className={`px-3 py-1.5 rounded-lg transition-all flex items-center gap-1.5 ${viewModes.batiment === 'table' ? 'active text-primary bg-white shadow-sm font-bold' : 'hover:text-slate-900'}`}
                  onClick={() => toggleView('batiment', 'table')}
                >
          ...
          {/* 2. Mode Visualisation Bulles 2D */}
          {viewModes.batiment === 'bubbles' && (
            <div className="card p-6">
              <BuildingsBubbleChart data={arrondissement3DData} />
            </div>
          )}

          {/* 3. Mode Visualisation 3D Three.js */}
          {viewModes.batiment === '3d' && (
            <div className="card p-6">
          ...
          {/* 4. Mode Tableau d'Entreprise */}
          {viewModes.batiment === 'table' && (
            <EnterpriseDataTable
