...
const TYPE_COLORS = [
  ...
  '#10b981', '#06b6d4', '#0ea5e9', '#3b82f6', '#4f46e5', '#6d28d9',
];
...
export default function TypesAnalyticsCharts({ rawData }) {
  const { barData, pieData, isDrillDown } = useMemo(() => {
    ...
    const totalVal = rawData.reduce((acc, curr) => acc + Number(curr.count || curr.total || 0), 0);

    const processedPie = rawData.map((d, index) => {
      const val = Number(d.count !== undefined ? d.count : d.total || 0);
      return {
        name: d.type || 'Non spécifié',
        value: val,
    ...
    const processedBar = rawData.map((d) => ({
      name: d.type || 'Non spécifié',
      'Volume Rénové': Number(d.count !== undefined ? d.count : d.total || 0),
      'Parc Total': Number(d.total || d.count || 0),
    }));
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
        <h3 className="text-sm font-bold text-slate-500 mb-6 uppercase tracking-wider">
          {isDrillDown
            ...
            : 'Types de Rénovation (Volume de Travaux)'}
        </h3>
        <div className="h-[380px] w-full">
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={barData} margin={{ top: 10, right: 10, left: 0, bottom: 35 }}>
              ...
              <Bar dataKey="Volume Rénové" fill="#8b5cf6" radius={[4, 4, 0, 0]} />
              {isDrillDown && <Bar dataKey="Parc Total" fill="#e2e8f0" radius={[4, 4, 0, 0]} />}
            </BarChart>
          </ResponsiveContainer>
        </div>
      </div>
      ...
      <div className="rounded-2xl border border-slate-200 bg-white p-8 shadow-sm">
        <h3 className="text-base font-bold text-slate-800 mb-6">Part des Types de Travaux</h3>
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-center">
          <div className="lg:col-span-5 relative h-[320px] flex items-center justify-center">
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
              <span className="text-xs font-bold text-slate-400 uppercase tracking-widest">TYPES</span>
