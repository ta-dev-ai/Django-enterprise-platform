...
export default function OverviewSection({ data, loading }) {
  ...
  const kpis = useMemo(() => {
    ...
    const totalLogements = buildings.reduce((a, b) => a + (b.total_logements || 0), 0);
    const totalRenoves = buildings.reduce((a, b) => a + (b.total_logements_renoves || 0), 0);
    const globalRate = totalLogements > 0 ? (totalRenoves / totalLogements) * 100 : 0;

    const topArrondissement = buildings.reduce((best, b) => {
      const rate = b.total_logements > 0 ? b.total_logements_renoves / b.total_logements : 0;
  ...
  useEffect(() => {
    ...
    const buildings = (data.buildings ?? []).slice().sort((a, b) => b.total_logements - a.total_logements).slice(0, 8);
    const barData = buildings.map((b) => ({ name: b.name, total: b.total_logements, renovated: b.total_logements_renoves }));
    ...
    if (barEl && barData.length > 0) {
      ...
      const c1 = new window.ApexCharts(barEl, {
        ...
        chart: { ...getBarOptions(barData, '').chart, height: 220, toolbar: { show: false } },
        legend: { show: false },
        title: { text: '' },
      });
    ...
    if (typesEl && typesShare.length > 0) {
      ...
      const c2 = new window.ApexCharts(typesEl, {
        ...getDonutOptions(typesShare, 'TYPES'),
        chart: { ...getDonutOptions(typesShare, 'TYPES').chart, height: 220 },
      });
    ...
    if (dpeEl && dpeShare.length > 0) {
      ...
      const c3 = new window.ApexCharts(dpeEl, {
        ...getDonutOptions(dpeShare, 'DPE'),
        chart: { ...getDonutOptions(dpeShare, 'DPE').chart, height: 220 },
      });
      ...
      <div className="flex flex-wrap gap-4 mb-8">
        ...
        <Link to="/batiment" className="card p-6 flex-1 min-w-[200px] hover:shadow-md transition-shadow">
          ...
          <div className="text-2xl font-bold text-slate-800">
            ...
            {kpis.topArrondissement && <span className="re-badge re-badge--accent ml-2">{(kpis.topArrondissement.rate * 100).toFixed(1)}%</span>}
          </div>
        </Link>
        ...
        <Link to="/dpe" className="card p-6 flex-1 min-w-[200px] hover:shadow-md transition-shadow">
          ...
          <div className="text-2xl font-bold text-slate-800">
            ...
            {kpis.topDpe && <span className="re-badge re-badge--accent ml-2">{(kpis.topDpe.rate * 100).toFixed(1)}%</span>}
          </div>
        </Link>
      </div>
